...
When **Weight** metric is selected: shows maximum weight per session (blue line), with average reps as a second line on a right Y-axis (orange). RPE secondary indicators are shown as before.

When **Reps** metric is selected: shows average reps per session as a single line (orange).

When **Volume** metric is selected: shows moved sum per session as a single line (purple), Y-axis labelled "kg moved".
...
- **WHEN** at least one set in that session for the selected exercise has an RPE value and Weight metric is active
...
- **THEN** each session is shown as a row with date, sets summary, and the selected metric value
